pipeline {
   ...
   parameters {
	...
	string(name: 'cluster', defaultValue : 'my-wordpress-cluster', description: "EKS cluster name")
    ...
    string(name: 'instance_type', defaultValue : 'c5.large', description: "k8s worker node instance type.")
  ...
  stages {
    stage('checkout') {
        steps {
            git 'https://github.com/aksk8/terrafrom_eks_wordpress.git'
        }
    }
	stage('Setup') {
		steps {
